...
export default function HeroSlider() {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden pt-16 transform-gpu">

      {/* =====================================================
          MOBILE-ONLY SEAM COVER (v2)

          The background layers below are `absolute inset-0`,
          so they start at the SAME y=0 as the fixed Navbar —
          they're just hidden behind it. The visible seam sits
          at the navbar's real rendered bottom edge (82 / 84 /
          92px depending on breakpoint), NOT at the top of this
          section (pt-16 does not shift absolutely-positioned
          children). This strip matches those exact navbar
          heights (+10px extra) and fades from navy to
          transparent so it blends into the hero image/overlay
          instead of showing as a hard block.

          Desktop (md and up) is completely untouched.
      ===================================================== */}
      <div
        className="
          pointer-events-none
      ...
      {/* ================= MOBILE BACKGROUNDS ================= */}
      <div className="absolute inset-0 block md:hidden transform-gpu">
        {slides.map((item, index) => (
          <div
            key={`mobile-${item.id}`}
            className="absolute inset-0 transition-opacity duration-700 ease-in-out"
